...
class CameraCapture(object):
    ...
    def __init__(
        ...
        if imageProcessingParams == "":
            ...
            self.imageProcessingParams = json.loads('{"visualFeatures":"tags"}')
        ...
        if self.computerVisionEndpoint_FULL:
            ...
            self.computerVisionHeader = {'Ocp-Apim-Subscription-Key': self.computerVisionKey,
    ...
    def __sendFrameForProcessing(self, frame):
        ...
        try:
            if self.computerVisionEndpoint_FULL:
                response = requests.post(self.computerVisionEndpoint_FULL, headers = self.computerVisionHeader, params = self.imageProcessingParams, data = frame)
            else:
    ...
    def start(self):
        ...
        while True:
            ...
            if self.imageProcessingEndpoint != "":
                ...
                ###########################
                # Send over ComputerVision for processing
                # if self.computerVisionEndpoint_FULL:
                #     response = self.client.analyze_image_in_stream(encodedFrame,visual_features=[VisualFeatureTypes.tags])
                # elif self.computerVisionEndpoint_FULL == "":
